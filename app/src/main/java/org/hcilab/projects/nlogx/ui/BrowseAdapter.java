...
class BrowseAdapter extends RecyclerView.Adapter<BrowseViewHolder> implements Filterable {
	...
	private ArrayList<DataItem> fullData = new ArrayList<>();
	private Set<DataItem> set = new HashSet<>();
	...
	public Filter filtered = new Filter() {
		...
		protected FilterResults performFiltering(CharSequence charSequence) {
			...
			if(charSequence == null || charSequence.length() == 0) {
				...
				for (DataItem dataItem : fullData){
					if(dataItem.getAppName().toLowerCase().startsWith(filterPattern) ||
							dataItem.getPackageName().toLowerCase().startsWith(filterPattern) ||
							dataItem.getText().toLowerCase().contains(filterPattern)){
		...
		protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
			...
			List<DataItem> non_duplicateList = new ArrayList<>();
